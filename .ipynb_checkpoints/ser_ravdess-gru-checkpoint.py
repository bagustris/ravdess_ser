...
from keras.layers import Dense, Activation, GRU, Flatten, LSTM 
...
earlystop = EarlyStopping(monitor='loss', mode='min', patience=50, restore_best_weights=True)
checkpointer = ModelCheckpoint(filepath='/tmp/weights.hdf5', verbose=1, save_best_only=True)

# function to define model
def create_model():  
    model = Sequential()  
    model.add(GRU(n_dim, input_shape=(1, 193), return_sequences=True, dropout=0.1, 
...
# train the model  
hist = model.fit(train_x, train_y, epochs=300, validation_data=[test_x, test_y], 
